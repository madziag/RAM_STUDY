...
### Creates empty df for expanding counts files (when not all month-year combinations have counts) - uses denominator file min and max year values
# Loads denominator file
denominator_retinoid<-as.data.table(readRDS(paste0(tmp,list.files(tmp,pattern=paste0(pop_prefix,"_denominator_retinoid.rds")))))[,studyFUmonths:=NULL]

# Split Y-M variable to year - month columns (for creating empty df & and getting min and max of data available)
denominator_retinoid[,c("year","month"):= tstrsplit(YM,"-",fixed=TRUE)]
denominator_retinoid[,year:=as.integer(year)][,month:=as.integer(month)]
# Get min and max data available from denominator_retinoid
min_data_available<-min(denominator_retinoid$year)
max_data_available<-max(denominator_retinoid$year)
# Create empty df using these min and max values
empty_df<-as.data.table(expand.grid(seq(min_data_available,max_data_available),seq(1,12)))
names(empty_df)<-c("year","month")
# Rearrange columns
denominator_retinoid<-denominator_retinoid[,c("YM", "year","month","Freq")]