...
## RAM_episodes - generated by RAM_treatment_episodes
## retinoid prevalence data 
## retinoid study denominator -> this is a subset of WOCBP that has received retinoid treatment at lease once in 
## between study start and study end - generated by denominator_monthly_retinoid_users.R
#### RETINOID PREVALENCE DATA IS USED HERE 
#############################################################################################
################################## Data P reparation #########################################
#############################################################################################
...
RAM_episodes<-merge(retinoid_prevalence_data_unique[,c("person_id","episode.start.retinoid")],RAM_episodes,by="person_id")
...
RAM_episodes_expanded<-RAM_episodes_expanded[episode.day>=episode.start.retinoid,]
...
RAM_prevalence<-RAM_prevalence[,-c("rowID","idnum","episode.start.retinoid")]
# Reorder columns
...
# Remove any records where episode day is before first record of Retinoid use (prevalence)
RAM_incidence<-RAM_incidence[episode.start>=episode.start.retinoid,]
...
# Remove any records where episode day is before first record of Retinoid use (prevalence)
RAM_discontinued<-RAM_discontinued[episode.end>=episode.start.retinoid]
# Creates year and month columns
