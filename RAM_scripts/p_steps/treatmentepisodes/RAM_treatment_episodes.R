...
## RAM meds generated by monthly_counts_ATC.R
########################################################################################################################
########################################################################################################################
########################################################################################################################
...
RAM_meds<-merge(retinoid_prevalence_data_unique[,c("person_id", "episode.start.retinoid", "episode.day.retinoid")],RAM_meds,by="person_id")

# Remove any rows where a patient has 0 RAM prescriptions after the first retinoid use (episode.start)
RAM_meds<-RAM_meds[, max_RAM_before_episode:=ifelse(max(Date)<episode.start.retinoid, 0, 1), by = person_id]
RAM_meds<-RAM_meds[max_RAM_before_episode==1,][,max_RAM_before_episode:=NULL]
